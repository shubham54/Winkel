...
<link href="css/styleoffer.css" rel="stylesheet" type="text/css" media="all" />
  ...
  <div class="main">
     <div class="wrap">
		<div class="cont span_2_of_3">
		  	<h2 class="head">OFFERS ZONE</h2>
            <?PHP
$query="SELECT * FROM products order by count desc";
$result=mysql_query($query);
$query1="SELECT * FROM products order by count asc";
$result1=mysql_query($query1);
$x=0;
while($row=mysql_fetch_array($result) and $row1=mysql_fetch_array($result1))
{
	$shui=$row['price']+$row1['price']-200;
	if($x == 15)
		break;
	if($x<9 && $x%3==0)
			...
			<div class="col_1_of_3 span_1_of_3" >
    		 <div class="inner_content clearfix">
				<div class="product_image">
					<img src="<?PHP echo $row['url'];?>" width="100px" height="100px" style="padding-left:30px; padding-top:30px;" alt=""/>
                    <img src="<?PHP echo $row1['url'];?>" width="100px" height="100px" alt=""/>
                </div>
               <div class="sale-box1"><span class="on_sale title_shop">Combo</span></div>	
                   <div class="price">
				   <div class="cart-left">
						<p><?PHP echo $row['product_name']; $flag=1;?> AND <?PHP echo $row1['product_name']; $flag=1;?> </p>
						<div class="price1">
                        <span class="reducedfrom"><per>Rs.</per><?PHP echo $row['price']+$row1['price'];?></span>
							  <span class="actual"><per>Rs.</per><?PHP echo $shui ;?></span>
						</div>
					</div>
					<div><form action="hell5.php" method="post">
                         <input type="hidden" value="<?PHP echo $row['id']?>" name="aa">
                         <input type="hidden" value="<?PHP echo $row1['id']?>" name="bb">
                          <input type="hidden" value="<?PHP echo $shui?>" name="cc">
